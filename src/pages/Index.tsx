  ...
  return (
    <div className="min-h-screen relative bg-ivory text-coffee">
      ...
      <main className="container mx-auto px-4">
        <section className="grid md:grid-cols-12 gap-10 items-center py-20 md:py-28">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }} className="md:col-span-7">
            <p className="uppercase tracking-[0.3em] text-xs text-taupe mb-6">Vol. 01 — A quiet practice</p>
            ...
            <div className="mt-10 flex gap-3 flex-wrap">
              <Button asChild size="lg"><Link to={user ? "/dashboard" : "/auth"}>{user ? "Open dashboard" : "Begin a session"}</Link></Button>
              <Button asChild variant="outline" size="lg"><Link to="/auth">Create an account</Link></Button>
            </div>
          </motion.div>

          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.8, delay: 0.2 }}
            className="md:col-span-5">
            <div className="editorial-panel p-8 bg-blush relative overflow-hidden">
              <div className="absolute -top-12 -right-10 w-44 h-44 rounded-full bg-clay/60 blur-2xl" />
              <p className="font-serif italic text-2xl leading-snug text-coffee/90">
